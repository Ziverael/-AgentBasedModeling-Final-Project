...
from typing import Any, Final, Optional, Union, List, Dict, Set
...
# --- Core Logic (Optimized Correctly) ---
...
class Location:
    ...
    def __init__(self, type: Place, size: Optional[int] = None) -> None:
        ...
        self.max_size = size
        self._type = type
        # CORRECT FIX: Use Deque (History) not Sets, to ensure overlap logic is valid
    ...
    def add_agent(self, agent: "Agent"):
        ...
        self._agent_count += 1
        # Add to history
        self._guests_list.append(GuestEntry(agent, agent.current.arrived_at, agent.current.stay_until))
    
    def remove_agent(self, agent: "Agent"):
        self._agent_count -= 1
        # No need to remove from deque explicitly, handled by lazy pop
...
class Agent:
    ...
    def __init__(self, state: AgentState, type: AgentType, is_multispreader: bool = False) -> None:
        ...
        # OPTIMIZATION: Neighbor Cache
        self._cached_neighbors: List[int] = []
        self._type = type
    ...
    @property
    def type(self) -> AgentType: return self._type
    @property
    def is_multispreader(self) -> bool: return self._is_multispreader
    @is_multispreader.setter
    def is_multispreader(self, val: bool) -> None: self._is_multispreader = val
    ...
    def change_current_place(self, rng: np.random.Generator) -> Optional[QueueEvent]:
        ...
        # Optimization: Native random choice for small list
        time_type = random.choice([TimeType.FREE, TimeType.HOME, TimeType.WORK])
        ...
        if time_type == TimeType.WORK:
            ...
            if self._free_destinations:
                # Optimization: Native random choice
                new_location_id = random.choice(self._free_destinations)
    ...
    def resolve_contact_wih_spreader(self, time: float, rng: np.random.Generator) -> Optional[QueueEvent]:
        # Optimization: Native random
        if random.random() <= Agent.transition_table.SE:
...
# --- World and Network (Cached Neighbors Optimization) ---
...
class EventEvaluator:
    ...
    def _resolve_contacts(self, agent: Agent, time: float) -> List[QueueEvent]:
        ...
        # Revert to DEQUE inspection for correctness
        ...
        if susceptible_ids:
            ...
            if count > 0:
                # Optimization: Native Random Sample
                contacted_ids = random.sample(susceptible_ids, k=count)
    ...
    def _resolve_contacts_as_susceptible(self, agent: Agent, time: float) -> List[QueueEvent]:
        ...
        if infectious_ids:
            ...
            if count > 0:
                # Optimization: Native Random Sample
                contacted = random.sample(infectious_ids, k=count)
                for _ in contacted:
    ...
    def _assign_locations(self):
        ...
        for agent in Agent.agents:
            ...
            try:
                ...
                agent._cached_neighbors = neighbors
                if neighbors:
