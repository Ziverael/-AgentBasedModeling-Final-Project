...
# OPTIMIZATION: Removed Pydantic, using slots for fast event processing
class QueueEvent:
...
class EventEvaluator:
    def __init__(self, world: World, rng: np.random.Generator, alpha: int) -> None:
        ...
        # OPTIMIZATION: Incremental state tracking
        self.state_counts: Counter[AgentState] = Counter()

    def init_stats(self):
        """Called after agents are created to populate initial stats"""
            ...
            if count > 0:
                ...
                for idx in contacted_ids:
                    ...
                    if target.state == AgentState.S:
                        # Capture old state for incremental stats
...
class Simulation:
    ...
    def init(self):
        ...
        self.world.init()
        self._init_agents()
        # Init stats after agents are created
