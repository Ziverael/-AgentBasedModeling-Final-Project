...
from typing import Any, Final, Optional, Union, List, Dict
...
class TransitionTable(BaseModel):
    ...
    SE: float = Field(default=0.01, le=1.0)
    I0I1: float = Field(default=0.4, le=1.0)
    I0I2: float = Field(default=0.6, le=1.0)
    I2R: float = Field(default=0.83, le=1.0)
    I2H: float = Field(default=0.17, le=1.0)
    HD: float = Field(default=0.049, le=1.0)
    HR: float = Field(default=0.951, le=1.0)

    @model_validator(mode="after")
    def check_sums(self):
        if not abs(self.I0I1 + self.I0I2 - 1.0) < 1e-5:
            raise ValueError(f"Sum of I0->I1 ({self.I0I1}) and I0->I2 ({self.I0I2}) must be 1.0.")
        if not abs(self.I2H + self.I2R - 1.0) < 1e-5:
            raise ValueError(f"Sum of I2->R ({self.I2R}) and I2->H ({self.I2H}) must be 1.0.")
        if not abs(self.HD + self.HR - 1.0) < 1e-5:
            raise ValueError(f"Sum of H->R ({self.HR}) and H->D ({self.HD}) must be 1.0.")
        return self
...
class AgentTypeRatio(BaseModel):
    ...
    def check_sum_is_one(self):
        total = self.retired + self.adult + self.child
...
class Params(BaseModel):
    T: PositiveInt = Field(..., description="Simulation max time in hours")
    N: PositiveInt = Field(..., description="Total population")
    
    # Infection Parameters
    alpha: PositiveInt = Field(..., description="Number of contacts per hour by multispreaders.")
    exposed: Union[float, PositiveInt] = Field(..., description="Initial exposed count (int) or ratio (float)")
    multispreader_share: float = Field(0.19, ge=0.0, le=1.0, description="Fraction of population that are multispreaders")
...
class Location:
    ...
    def add_agent(self, agent: "Agent"):
        if self.max_size is not None and len(self._agents) >= self.max_size:
    ...
    def inspect_guests_list(self, from_: float, to_: float) -> List["Agent"]:
        if self._guests_list:
            self._guests_list = [g for g in self._guests_list if g.to_ >= from_]
            
        return list({
            guest_entry.agent 
            for guest_entry in self._guests_list 
            if from_ <= guest_entry.from_ <= to_ or
               from_ <= guest_entry.to_ <= to_ or
               (guest_entry.from_ <= from_ and guest_entry.to_ >= to_)
        })
...
class Agent:
    ...
    def is_multispreader(self, val: bool) -> None:
        self._is_multispreader = val
    ...
    def add_travel_destination(self, dest_id: Union[int, List[int], NDArray[np.int64]]) -> None:
        if isinstance(dest_id, int):
            self._travel_destinations.append(dest_id)
             ...
             self._travel_destinations.extend(list(dest_id))
    ...
    def change_current_place_deterministically(self, location_id: int, duration: float) -> QueueEvent:
        ...
        self._change_current_state(
            AgentCurrentState(location_id=location_id, arrived_at=arrived_time, stay_until=left_time)
        )
        return QueueEvent(time=left_time, agent=self, type=EventType.CHANGE_PLACE)

    def change_current_place(self, rng: np.random.Generator) -> Optional[QueueEvent]:
        if self._state in AgentState.RESTRICTED:
            return self._apply_state_rules_to_movement(rng)
        
        time_type = rng.choice(list(TimeType))
        ...
        match time_type:
            ...
            case TimeType.FREE:
                valid = [loc_id for loc_id in self._travel_destinations 
                         if Location.locations[loc_id].type in Place.FREE_TIME]
                new_location_id = rng.choice(valid) if valid else self.house_id
            case _:
                new_location_id = self.house_id
    ...
    def _apply_state_rules_to_movement(self, rng: np.random.Generator) -> Optional[QueueEvent]:
        duration_home = np.abs(rng.logistic(loc=Agent.avg_time_table[TimeType.HOME]))
        match self.state:
            case AgentState.I2:
    ...
    def resolve_contact_wih_spreader(self, time: float, rng: np.random.Generator) -> Optional[QueueEvent]:
        if rng.random() <= Agent.transition_table.SE:
            return self.resolve_state_conversion(time, rng)
        return None

    def resolve_state_conversion(self, current_time: float, rng: np.random.Generator) -> Optional[QueueEvent]:
        ...
        match self.state:
            case AgentState.S:
                ...
                activation_time = np.abs(rng.logistic(loc=st.E))
            case AgentState.E:
                self._state = AgentState.I0
                activation_time = np.abs(rng.logistic(loc=st.I0))
            case AgentState.I0:
                if rng.random() < tt.I0I1:
                    self._state = AgentState.I1
                    activation_time = np.abs(rng.logistic(loc=st.I1))
                else:
                    self._state = AgentState.I2
                    activation_time = np.abs(rng.logistic(loc=st.I2))
            case AgentState.I1:
            ...
            case AgentState.I2:
                if rng.random() < tt.I2R:
                    ...
                    activation_time = np.abs(rng.logistic(loc=st.H))
            case AgentState.H:
                if rng.random() < tt.HR:
                    self._state = AgentState.R
                else:
...
class EventEvaluator:
    def __init__(self, world: World, rng: np.random.Generator, alpha: int) -> None:
        self._world = world
        self.rng = rng
        self._alpha = alpha
    ...
    def _resolve_contacts(self, agent: Agent, time: float) -> List[QueueEvent]:
        ...
        agents_in_loc = loc.inspect_guests_list(agent.current.arrived_at, agent.current.stay_until)
        ...
        susceptible_ids = [a._id for a in agents_in_loc if a._id != agent._id and a.state == AgentState.S]
        
        events = []
        if susceptible_ids:
            count = min(len(susceptible_ids), interactions_no)
            if count > 0:
                contacted_ids = self.rng.choice(susceptible_ids, size=count, replace=False)
                for idx in contacted_ids:
                    target = Agent.agents[idx]
    ...
    def _resolve_contacts_as_susceptible(self, agent: Agent, time: float) -> List[QueueEvent]:
        ...
        infectious_ids = [a._id for a in agents_in_loc if a._id != agent._id and a.state in AgentState.INFECTIOUS]
        
        events = []
        if infectious_ids:
            interactions_no = int(agent.current.stay_until - agent.current.arrived_at)
            count = min(len(infectious_ids), interactions_no)
            if count > 0:
                contacted = self.rng.choice(infectious_ids, size=count)
                for _ in contacted:
    ...
    def _eval_get_stats(self, event: QueueEvent) -> List[QueueEvent]:
        counts = Counter(a.state for a in Agent.agents)
...
class Simulation:
    ...
    def _init_exposed_agents(self):
        ...
        if isinstance(total_exposed, float):
            total_exposed = int(self.params.N * total_exposed)
        
        exposed_ids = self.rng.choice(len(Agent.agents), size=total_exposed, replace=False)
    ...
    def _set_multipreaders(self) -> None:
        ...
        if ms_no > 0:
            ms_ids = self.rng.choice(len(Agent.agents), size=ms_no, replace=False)
            for idx in ms_ids:
    ...
    def _assign_locations(self):
        ...
        if hosp_ids:
            assignments = self.rng.choice(hosp_ids, size=len(Agent.agents))
        ...
        if school_ids:
            ...
            if kids:
                assigns = self.rng.choice(school_ids, size=len(kids))
        ...
        if work_ids:
            ...
            if adults:
                assigns = self.rng.choice(work_ids, size=len(adults))
        ...
        for agent in Agent.agents:
            ...
            if care_ids:
                agent.add_travel_destination(self.rng.choice(care_ids, size=min(3, len(care_ids)), replace=False))
            if pub_ids:
                agent.add_travel_destination(self.rng.choice(pub_ids, size=min(5, len(pub_ids)), replace=False))
            
            try:
                neighbors = [n for n in city_graph.neighbors(agent.house_id) if Location.locations[n].type == Place.HOUSEHOLD]
                if neighbors:
                    agent.add_travel_destination(self.rng.choice(neighbors, size=min(4, len(neighbors)), replace=False))
            except Exception:
                pass
    ...
    def _create_initial_events(self) -> None:
        for agent in Agent.agents:
            ...
            if agent.state == AgentState.E:
                time = np.abs(self.rng.logistic(loc=Agent.state_time_table.E))
                self.event_queue.put(QueueEvent(time=time, agent=agent, type=EventType.CHANGE_STATE))
    ...
    def _init_public_events(self):
        ...
        for name, p_event in self._public_events_map.items():
            ...
            agents = self.rng.choice(Agent.agents, size=k)
            k_re = int(p_event.composition * self.params.N)
            reappearing = self.rng.choice(agents, size=k_re) if k_re > 0 else []
            ...
            starts = self.rng.uniform(0, 7*DAY_HOURS, size=len(agents))
