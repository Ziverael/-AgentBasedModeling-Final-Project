...
class Place(Flag):
    ...
    # Helper for essential checks
    ESSENTIAL_WORK = HOSPITAL | PRIMARY_CARE
...
class EventType(Enum):
    ...
    LOCKDOWN = auto()
    GET_STATS = auto()
    SCENARIO_UPDATE = auto()  # NEW: Event to trigger policy changes

# --- Config ---
class TransitionTable(BaseModel):
    model_config = ConfigDict(frozen=True)
    SE: float = Field(default=0.014)
...
class Params(BaseModel):
    ...
    initial_infected_normal: NonNegativeInt = Field(default=0)
    initial_infected_multispreader: NonNegativeInt = Field(default=0)
...
# --- NEW: NPI Policy Configuration ---
class InterventionPolicy(BaseModel):
    model_config = ConfigDict(frozen=True)
    
    # 1. Masks & Social Distancing
    masks_enabled: bool = False
    mask_efficiency: float = 0.48 # Reduces S->E probability by 48%
    
    # 2. No Social Events
    social_events_banned: bool = False
    
    # 3. Partial Lockdown (School + Selected Public Places closed)
    school_closure: bool = False
    public_places_closed: bool = False 
    
    # 4. Work-From-Home (Non-essential only)
    wfh_enabled: bool = False
    wfh_non_essential_fraction: float = 0.0
    
    # 5. Complete Lockdown (Non-essential work closed + Schools + Public)
    non_essential_workplaces_closed: bool = False

    @staticmethod
    def normal() -> "InterventionPolicy":
...
class QueueEvent:
    __slots__ = ('time', 'type', 'agent', 'name', 'payload')
    def __init__(self, time: float, type: EventType, agent: Optional["Agent"] = None, name: Optional[str] = None, payload: Any = None):
        self.time = time
        self.type = type
        self.agent = agent
        self.name = name
        self.payload = payload # Modified to carry policy objects if needed
...
class Agent:
    ...
    # NEW: Global policy reference
    intervention_policy: InterventionPolicy = InterventionPolicy()
    ...
    def reset() -> None:
        Agent.agents = []
        Agent.intervention_policy = InterventionPolicy()
    ...
    def change_current_place(self, rng: BatchRNG) -> Optional[QueueEvent]:
        ...
        policy = Agent.intervention_policy
        ...
        # 2. Apply NPI Rules to Intention (Modifies time_type or redirects)
        
        # Rule: Schools
        if time_type == TimeType.WORK and self.type == AgentType.CHILD:
            if policy.school_closure:
                time_type = TimeType.HOME

        # Rule: Workplace Closure / WFH
        if time_type == TimeType.WORK and self.work_id is not None:
            # Check type of work
            loc_type = Location.locations[self.work_id].type
            is_essential = loc_type in Place.ESSENTIAL_WORK
            
            # Complete Lockdown logic (Close non-essential)
            if policy.non_essential_workplaces_closed and not is_essential:
                time_type = TimeType.HOME
            
            # WFH Policy (Partial fraction of non-essential)
            if time_type == TimeType.WORK and policy.wfh_enabled and not is_essential:
                # Deterministic check per agent to maintain consistency
                # (Agent 10 always WFH if fraction > 0.01 etc)
                check_val = (self._id * 1337) % 1000 / 1000.0
                if check_val < policy.wfh_non_essential_fraction:
                    time_type = TimeType.HOME
        ...
        if time_type == TimeType.WORK:
            ...
            # Filter destinations based on closures
            available_dests = []
            if self._free_destinations:
                for dest_id in self._free_destinations:
                    l_type = Location.locations[dest_id].type
                    
                    # Lockdowns on Public Places
                    if policy.public_places_closed and l_type == Place.PUBLIC_PLACE:
                        continue
                        
                    # "Complete Lockdown" usually implies reducing time/visits, 
                    # effectively covered by closing public places.
                    # Primary Care remains open.
                    
                    available_dests.append(dest_id)
            
            if available_dests:
                new_location_id = rng.choice(available_dests)
            else:
                # If everything closed or no friends, go home
                new_location_id = self.house_id
    ...
    def resolve_contact_wih_spreader(self, time: float, rng: BatchRNG) -> Optional[QueueEvent]:
        # MODIFIED: Mask Logic
        # Apply mask efficiency if enabled
        transmission_prob = Agent.transition_table.SE
        if Agent.intervention_policy.masks_enabled:
            transmission_prob *= (1.0 - Agent.intervention_policy.mask_efficiency)
...
class EventEvaluator:
    ...
    def eval(self, event: QueueEvent) -> List[QueueEvent]:
        match event.type:
            ...
            case EventType.SCENARIO_UPDATE:
                return self._eval_scenario_update(event)
            case _:
                return []
    ...
    def _eval_world_event(self, event: QueueEvent) -> List[QueueEvent]:
        ...
        # MODIFIED: Social Event NPI check
        if Agent.intervention_policy.social_events_banned:
            # If banned, we must ensure the agent doesn't get stuck.
            # We treat this as if the agent finished a previous activity and needs to move.
            # Call standard change_current_place to decide next move (Home/Free).
            move_event = event.agent.change_current_place(self.rng)
            if move_event:
                events.append(move_event)
            # Re-schedule the next occurrence of this event regardless of ban (it might be lifted later)
            pub_event = self.public_events_details[event.name]
            next_time = event.time + pub_event.frequency
            next_agent = event.agent if event.agent in pub_event.reappearing_agents else self.rng.choice(Agent.agents)
            events.append(QueueEvent(name=pub_event.name, time=next_time, agent=next_agent, type=EventType.WORLD_EVENT))
            return events
    ...
    def _eval_scenario_update(self, event: QueueEvent) -> List[QueueEvent]:
        if event.payload and isinstance(event.payload, InterventionPolicy):
            Agent.intervention_policy = event.payload
            # Log or print for debugging
            # print(f"[{event.time:.1f}] Scenario Updated: {event.name}")
        return []
...
class Simulation:
    ...
    # NEW: Helper to schedule interventions
    def schedule_intervention(self, time: float, policy: InterventionPolicy, name: str = "Policy Update"):
        self.event_queue.put(QueueEvent(
            time=time, 
