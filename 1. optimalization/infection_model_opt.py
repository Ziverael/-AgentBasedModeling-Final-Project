...
# (These remain Pydantic as they are only used for setup, not the hot loop)
...
# --- Core Logic Classes (OPTIMIZED) ---

# OPTIMIZATION 1: Removed Pydantic from hot-path class
...
class GuestEntry:
    __slots__ = ('agent', 'from_', 'to_')
    def __init__(self, agent: "Agent", from_: float, to_: float):
        self.agent = agent
        self.from_ = from_
        self.to_ = to_
...
class Location:
    ...
    def __init__(self, type: Place, size: Optional[int] = None) -> None:
        ...
        self._type = type
        # OPTIMIZATION 2: Use deque for O(1) pops from the start
        self._guests_list: deque[GuestEntry] = deque()
    ...
    def inspect_guests_list(self, from_: float, to_: float) -> List["Agent"]:
        # OPTIMIZATION 2: Efficiently pop old history
        while self._guests_list and self._guests_list[0].to_ < from_:
            self._guests_list.popleft()
            
        return list({
            entry.agent 
            for entry in self._guests_list 
            if from_ <= entry.from_ <= to_ or
               from_ <= entry.to_ <= to_ or
               (entry.from_ <= from_ and entry.to_ >= to_)
        })
...
class Agent:
    ...
    def __init__(self, state: AgentState, type: AgentType, is_multispreader: bool = False) -> None:
        ...
        # OPTIMIZATION 3: Pre-calculate free destinations
    ...
    def add_travel_destination(self, dest_id: Union[int, List[int], NDArray[np.int64]]) -> None:
        if isinstance(dest_id, int):
            ...
            # OPTIMIZATION 3: Check once at setup, not every step
    ...
    def change_current_place_deterministically(self, location_id: int, duration: float) -> QueueEvent:
        ...
        self._change_current_state(
            AgentCurrentState(location_id, arrived_time, left_time)
        )
    ...
    def change_current_place(self, rng: np.random.Generator) -> Optional[QueueEvent]:
        ...
        # Optimization: Manual list for speed instead of list(TimeType)
        ...
        if time_type == TimeType.WORK:
            ...
            # OPTIMIZATION 3: Use pre-calculated list
...
# --- World and Network (Identical logic, just standard python improvements) ---
...
class EventEvaluator:
    ...
    def _resolve_contacts(self, agent: Agent, time: float) -> List[QueueEvent]:
        # Optimization 4: Cache global lookup
        loc_list = Location.locations 
        if agent.current.location_id == -1: return []
        
        loc = loc_list[agent.current.location_id]
