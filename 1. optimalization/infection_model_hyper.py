...
class Location:
    ...
    def __init__(self, type: Place, size: Optional[int] = None) -> None:
        ...
        self.max_size = size
        self._type = type
        # Store tuples: (Agent, arrived, depart)
    ...
    def get_contacts(self, from_: float, to_: float, target_state: Optional[AgentState] = None) -> List[Agent]:
        """
        Merged guest inspection and filtering.
        Prunes old history and returns agents overlapping with [from_, to_].
        Optionally filters by state immediately.
        """
        # Prune old history
...
class Agent:
    ...
    # Direct attribute access (no properties) for speed
    __slots__ = (
    ...
    def __init__(self, state: AgentState, type: AgentType, is_multispreader: bool = False) -> None:
        ...
        self.state = state  # Public attribute, no @property overhead
    ...
    def resolve_state_conversion(self, current_time: float, rng: BatchRNG) -> Optional[QueueEvent]:
        ...
        # Direct attribute access for state
...
class EventEvaluator:
    ...
    def _resolve_contacts(self, agent: Agent, time: float) -> List[QueueEvent]:
        ...
        # Merge guest inspection and state filtering for speed
        # Get only Susceptible agents overlapping in time
    ...
    def _resolve_contacts_as_susceptible(self, agent: Agent, time: float) -> List[QueueEvent]:
        ...
        # We need ALL infectious agents
        # This is tricky because "INFECTIOUS" is a set of states, not a single state.
        # So we get all agents and filter manually (or we could improve get_contacts to accept a set)
...
class Simulation:
    ...
    def _init_exposed_agents(self):
        ...
        exposed_ids = self.rng.choice(list(range(len(Agent.agents))))
        exposed_ids = self.rng.sample(list(range(len(Agent.agents))), k=total_exposed)
    ...
    def _create_initial_events(self) -> None:
        for agent in Agent.agents:
            ...
            if agent.state == AgentState.E:
                time = self.rng.get_logistic(loc=Agent.state_time_table.E)
